...
//  chatgpt
//
//  Created by Burhan AFŞAR on 6.12.2023.
//

import SwiftUI
import MarkdownUI

//Markdown for chatText based github. Changed codeBlockBackgroundColor.
extension Theme{
      ...
      .codeBlock { configuration in
          VStack(spacing: 0){
              HStack{
                  ...
                  Text(configuration.language?.lowercased().capitalized ?? "")
                      .fontWeight(.semibold)
                  Spacer()
                  Button{
                      NSPasteboard.general.clearContents()
                      NSPasteboard.general.setString(configuration.content, forType: .string)
                      print(configuration.language)
                  }label: {
              ...
              }.padding(.horizontal).background(.black.opacity(0.4))
...
extension Color {
  ...
  fileprivate static let secondaryBackground = Color(
    ...
    fileprivate static let codeBlockBackground = Color(
      light: Color("BackgroundCode"), dark: Color("BackgroundCode")
    )
